...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--port", type=int, default=None, help="Bind port on the server")
    ...
    parser.add_argument("--ca", default=None, help="CA certificate that signed allowed client certs")
    parser.add_argument("--base-dir", default=None, help="Allowed working directory root")
    parser.add_argument("--allow-shell", action="store_true", default=None, help="Allow shell-mode commands")
    parser.add_argument("--timeout", type=int, default=None, help="Default command timeout")
    parser.add_argument("--max-output", type=int, default=None, help="Max stdout/stderr chars returned")
    parser.add_argument("--log", default=None, help="Audit log path")
    parser.add_argument("--strict-x509", action="store_true", default=None, help="Enable Python's strict X509 verification flags")
    args = parser.parse_args()

    config, config_path = load_json_config(args.config)
    args.host = option_value(args, config, "host", "0.0.0.0")
    ...
    args.base_dir = option_value(args, config, "base_dir", str(Path.cwd()))
    args.allow_shell = bool_option(parser, args, config, "allow_shell", False)
    args.timeout = int_option(parser, args, config, "timeout", DEFAULT_TIMEOUT_SECONDS)
    args.max_output = int_option(parser, args, config, "max_output", 4 * 1024 * 1024)
    args.log = resolve_local_path(option_value(args, config, "log", "remote_exec_server.log"), config_path)
    args.strict_x509 = bool_option(parser, args, config, "strict_x509", False)

    for name in ("port", "cert", "key", "ca"):
